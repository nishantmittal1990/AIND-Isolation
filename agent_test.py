...
class CounterBoard(isolation.Board):
    ...
    def copy(self):
        ...
        new_board._board_state = [CounterBoard.BLANK] * (self.width * self.height + 3)
        new_board._board_state[-1] = CounterBoard.NOT_MOVED
        new_board._board_state[-2] = CounterBoard.NOT_MOVED
...
class Project1Test(unittest.TestCase):
    ...
    def initAUT(self, depth, eval_fn, loc1=(3, 3), loc2=(0, 0), w=7, h=7):
    ...
    def test_minimax_interface(self):
        ...
        #Unit test for testing minimax interface -- Test1

        h, w = 7, 7  # board size
        test_depth = 1 #Defined some basic test
        starting_location = (5, 3) #Defines starting location for game player
        adversary_location = (0, 0)  #Defines starting location for opposite player
        #iterative_search = False
        #search_method = "minimax"
        heuristic = lambda g, p: 0.  # return 0 everywhere
        oppositePlayer = 'null_agent'
        # create a player agent & a game board
        gamePlayer = game_agent.MinimaxPlayer(
            test_depth, heuristic)
        gamePlayer.time_left = lambda: 99  # ignore timeout for fixed-depth search
        gameObject = isolation.Board(gamePlayer, oppositePlayer, w, h)

        # place two "players" on the board at arbitrary (but fixed) locations
        gameObject.apply_move(starting_location)
        #legal_moves = game.get_legal_moves ()
        gameObject.apply_move(adversary_location)

        for move in gameObject.get_legal_moves():
            next_state = gameObject.forecast_move(move)
            best_move = gamePlayer.minimax(next_state, test_depth)
            pass
            #print("Best Move from minimax interface testing is :: ",best_move)
            '''
            self.assertIn (best_move, legal_moves,
                            ("Assert-in condition has been written" +
                             "for move lie in legal moves in case of minimax interface testing"))
            '''
    ...
    def test_minimax(self):
        ...
        on the location of the search agent on the board to force minimamx to
    ...
    def test_alphabeta(self):
        ...
        for idx in range(len(counts)):
            ...
            self.assertTrue(num_explored_valid, WRONG_NUM_EXPLORED.format(
                test_depth, counts[idx][0], board.counts[0]))

            self.assertTrue(num_unique_valid, UNEXPECTED_VISIT.format(
                test_depth, counts[idx][1], board.counts[1]))

            self.assertIn(move, first_branch, WRONG_MOVE.format(
                test_depth, first_branch, move))
            '''
